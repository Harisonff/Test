label fightuna:
    ...
    yna "Alright, moosy, where's the kid?"
    play sound "audio/tiger.mp3"
    "mooselion" "aaarrrhhhh!"
    yna "don't make me come over there!"
    if povname == "jack":
...
label fight_mainuna:
    ...
    call screen fight_buttonsuna




label mooses_turnuna:
    play sound "audio/tiger.mp3"
    if charge:
        $ charge = False
        ...
        if moose_decides == 1:
            if blocking:
                $ blocking = False
                jump moose_attack_guarduna
            else:
                jump moose_attackuna
...
label main_attackuna:
    $ blocking = False
    if moose_blocking:
        $ moose_blocking = False
        pause 1
        jump moose_defendsuna
    else:
        jump avatar_attackuna
...
screen fight_buttonsuna:
    imagemap:
        ...
        hotspot (690, 570, 67, 147) action [Jump("main_attackuna")]
        hotspot (760, 570, 67, 147) action [Jump("main_blockuna")]
        hotspot (830, 570, 67, 147) action [Jump("main_potionuna")]
...
label moose_lostuna:
    ...
    hide screen hp_bar
    hide screen hp_bar9
    hide m_battle_stance01 with dissolve
    hide m_battle_small_01_hurt
    hide m_battle_stance02
    jump fightuna2
